...
public class DeleteServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(new QuestionDAO().setQuestionStatus(id, status)){
                if(status == 0)
                    request.setAttribute("message", "Remove Question successfully!!");
                else
            ...
            request.getRequestDispatcher(action.getProperty("SearchQuestion")).forward(request, response);
        }
    }
